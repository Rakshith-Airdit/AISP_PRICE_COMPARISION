sap.ui.define([
    ...
    return Controller.extend("com.pricecomp.pricecomp.controller.CreateServiceRFQ", {

        onInit: function () {
            this.router = this.getOwnerComponent().getRouter();
            this.router.getRoute("RouteCreateServiceRFQ").attachPatternMatched(this.onRouteMatched, this);
        },
        ...
        onRouteMatched: function () {
            ...
            this.byId("idRFQSRVBasicDetails").setBindingContext(this.basicDetailsContext);
        },

        onAddItem: function () {
            const itemsTable = this.byId("idRFQSRVItemsTable");
            ...
            const itemContext = model.createEntry("/ZC_RFQItemDetails", {
                properties: {
                    ...
                    quantity: 1,
                    UnitOfMeasure: "",
                    ExpectedServicePeriodStart: null,
                    ExpectedServicePeriodEnd: null
                }
            });
            ...
            const newRow = new sap.m.ColumnListItem({
                cells: [
                    ...
                    new sap.m.DateRangeSelection({
                        dateValue: "{ExpectedServicePeriodStart}",
                        secondDateValue: "{ExpectedServicePeriodEnd}",
                        placeholder: "Enter Expected Service Period",
                        required: true,
                        change: this.onServicePeriodChange.bind(this)
                    })
                ]
            });
        ...
        onMaterialDescriptionChange: function (event) {
            ...
            model.read("/ZC_AISP_MATERIALDETAILS_VH", {
                filters: [new Filter("Description", FilterOperator.EQ, materialDescription)],
                success: function (data) {
                    if (data.results.length > 0) {
                        const selectedMaterial = data.results[0];
                        context.setProperty("LotType", selectedMaterial.LotType || "Group");
        ...
        onServicePeriodChange: function (event) {
            const datePicker = event.getSource();
            const startDate = datePicker.getDateValue();
            const endDate = datePicker.getSecondDateValue();

            if (startDate && endDate && startDate > endDate) {
                datePicker.setValueState(sap.ui.core.ValueState.Error);
                datePicker.setValueStateText("End date cannot be before start date.");
            } else {
        ...
        onAddSupplier: function () {
            const suppliersTable = this.byId("SRVsuppliersTable");
            const model = this.getView().getModel();
        ...
        validateRFQData: function (headerData) {
            ...
            // Items validation
            const items = this.byId("idRFQSRVItemsTable").getItems();
            if (items.length === 0) {
                ...
                items.forEach(function (item, index) {
                    ...
                    // Replace the delivery date validation with:
                    if (!itemData.ExpectedServicePeriodStart || !itemData.ExpectedServicePeriodEnd) {
                        errors.push(`Row ${index + 1}: Both start and end dates are required.`);
                    } else if (itemData.ExpectedServicePeriodStart > itemData.ExpectedServicePeriodEnd) {
                        errors.push(`Row ${index + 1}: End date cannot be before start date.`);
                    }
                });
            }

            // Suppliers validation
            const suppliers = this.byId("SRVsuppliersTable").getItems();
        ...
        buildRFQPayload: function (headerData) {
            // Build items payload
            const itemsPayload = this.byId("idRFQSRVItemsTable").getItems().map(function (item, index) {
                const context = item.getBindingContext();
                const itemData = context ? context.getModel().getObject(context.getPath()) : null;
                ...
                return {
                    ItemNo: itemNumber,
                    Matnr: itemData.Matnr || null,
                    Description: itemData.Description || "",
                    Werks: itemData.Werks || null,
                    Name1: itemData.Name1 || "",
                    quantity: itemData.quantity,
                    UnitOfMeasure: itemData.UnitOfMeasure || "",
                    // Replace the DeliveryDate property with:
                    ExpectedServicePeriodStart: itemData.ExpectedServicePeriodStart || null,
                    ExpectedServicePeriodEnd: itemData.ExpectedServicePeriodEnd || null
                };
            }).filter(function (item) { return item !== null; });

            // Build suppliers payload
            const suppliersPayload = this.byId("SRVsuppliersTable").getItems().map(function (supplier) {
                const context = supplier.getBindingContext();
                const supplierData = context ? context.getModel().getObject(context.getPath()) : null;
                if (!supplierData) return null;

                return {
                    Lifnr: supplierData.Lifnr || null,
                    Name1: supplierData.VendorName || ""
                };
            ...
            return {
                ...
                RFQType: "Service",
                RFQToItem: { results: itemsPayload },
                RFQToSupplier: { results: suppliersPayload },
            };
        },
