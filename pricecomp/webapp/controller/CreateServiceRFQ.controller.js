sap.ui.define(
  ...
  function (
    ...
    return Controller.extend(
      "com.pricecomp.pricecomp.controller.CreateServiceRFQ",
      {
        onInit: function () {
          this.router = this.getOwnerComponent().getRouter();
          this.router
            .getRoute("RouteCreateServiceRFQ")
        ...
        onRouteMatched: function () {
          ...
          this.byId("idRFQSRVBasicDetails").setBindingContext(
            this.basicDetailsContext
          );
        },
        ...
        onMaterialDescriptionChange: function (event) {
          ...
          model.read("/ZC_AISP_MATERIALDETAILS_VH", {
            filters: [
              new Filter("Description", FilterOperator.EQ, materialDescription),
            ],
            success: function (data) {
              if (data.results.length > 0) {
                ...
                context.setProperty(
                  "LotType",
                  selectedMaterial.LotType || "Group"
                );
                context.setProperty(
                  "UnitOfMeasure",
        ...
        validateRFQData: function (headerData) {
          ...
          if (!headerData.quotationDeadline)
          ...
          if (items.length === 0) {
            ...
            items.forEach(function (item, index) {
              ...
              const itemData = context
                ? context.getModel().getObject(context.getPath())
                : null;
              if (!itemData) return;
              ...
              if (!itemData.quantity || Number(itemData.quantity) <= 0)
                errors.push(`Row ${index + 1}: Quantity must be > 0.`);
              if (!itemData.UnitOfMeasure)
                errors.push(`Row ${index + 1}: UoM is required.`);
              // Replace the delivery date validation with:
              if (
                !itemData.ExpectedServicePeriodStart ||
                !itemData.ExpectedServicePeriodEnd
              ) {
                errors.push(
                  `Row ${index + 1}: Both start and end dates are required.`
                );
              } else if (
                itemData.ExpectedServicePeriodStart >
                itemData.ExpectedServicePeriodEnd
          ...
          if (suppliers.length === 0) {
            ...
            suppliers.forEach(function (supplier, index) {
              ...
              const supplierData = context
                ? context.getModel().getObject(context.getPath())
                : null;
              if (!supplierData) return;

              if (!supplierData.Lifnr)
                errors.push(
                  `Supplier row ${index + 1}: Supplier Code is required.`
                );
            });
          }
        ...
        buildRFQPayload: function (headerData) {
          // Build items payload
          const itemsPayload = this.byId("idRFQSRVItemsTable")
            .getItems()
            .map(function (item, index) {
              ...
              return {
                ...
                ExpectedServicePeriodStart:
                  itemData.ExpectedServicePeriodStart || null,
                ExpectedServicePeriodEnd:
                  itemData.ExpectedServicePeriodEnd || null,
              };
            })
          ...
          const suppliersPayload = this.byId("SRVsuppliersTable")
            .getItems()
          ...
          try {
            ...
            model.callFunction("/createRFQ", {
              ...
              success: function (data) {
                ...
                this._setBusy(false);
                MessageBox.success(
                  `RFQ with number ${rfqNumber} created successfully.`
                );
        ...
        resetAfterSubmit: function () {
          ...
          if (this.basicDetailsContext) {
            try {
              model.deleteCreatedEntry(this.basicDetailsContext);
            } catch (e) {}
